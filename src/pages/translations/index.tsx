...
type ColumnType = ColumnDef<TranslationInterface, unknown>;
...
export function TranslationListPage(props: TranslationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTranslations({
        ...
        order: params.order,
        searchTermKeys: ['language.contains', 'translated_text.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TranslationInterface>>(
    () => `/translations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TranslationInterface) => {
    if (hasAccess('translation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/translations/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'language', header: 'Language', accessorKey: 'language' },
    { id: 'translated_text', header: 'Translated Text', accessorKey: 'translated_text' },
    hasAccess('news', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/news/view/${record.news?.id}`}>
              {record.news?.title}
            </Link>
          ),
        }
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/translations/view/${record.id}`} passHref legacyBehavior>
            <Button
